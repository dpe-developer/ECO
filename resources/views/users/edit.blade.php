<form method="POST" action="{{ route('users.update', $user->id) }}" autocomplete="off" enctype="multipart/form-data">
	@csrf
	@method('PUT')
	<input type="hidden" name ="from_modal_ajax_href" value="{{ route('users.edit', $user->id) }}">
	<input type="hidden" name ="modal_ajax_target" value="#editUser">
						...
						<div class="row">
							<div class="col">
								...
								<div class="form-group">
									...
									<select style="width: 100%" class="form-control select2{{ $errors->has('role') ? ' is-invalid' : '' }}" name="role">
										...
										@foreach ($roles as $role)
											<option @if(old('role') == $role->id){{'selected'}}@elseif(($user->role->role_id ?? 0) == $role->id){{'selected'}}@endif value="{{ $role->id }}">
												{{ $role->name }}
											</option>
										@endforeach
									</select>
								...
								<div class="form-group">
									...
									<input id="first_name" value="@if(old('first_name')){{ old('first_name') }}@else{{ $user->first_name }}@endif" type="text" class="form-control{{ $errors->has('first_name') ? ' is-invalid' : '' }}" name="first_name">
									<span class="invalid-feedback" role="alert">
								...
								<div class="form-group">
									...
									<input id="last_name" value="@if(old('last_name')){{ old('last_name') }}@else{{ $user->last_name }}@endif" type="text" class="form-control{{ $errors->has('last_name') ? ' is-invalid' : '' }}" name="last_name">
									<span class="invalid-feedback" role="alert">
